...
const mallContext = vi.hoisted(() => ({
  selectedMallId: null as string | null,
}));
...
describe("ServiceContractsPage all-mall view", () => {
  beforeEach(() => {
    ...
    vi.mocked(serviceContractsApi.updateStatus).mockResolvedValue({
      id: "contract-created",
      status: "PROPOSAL",
    } as never);
  });
  ...
  it("loads contracts and alerts without a mallId while keeping creation mall-specific", async () => {
    ...
    expect(
      screen.getByText(/Đang xem tổng hợp hợp đồng của tất cả Mall/),
    ).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "create" })).toBeDisabled();

    await waitFor(() => {
      expect(serviceContractsApi.list).toHaveBeenCalledWith(
        expect.objectContaining({ mallId: undefined }),
      );
      expect(serviceContractsApi.alerts).toHaveBeenCalledWith(30, undefined);
    });
  });

  it("renders contract detail as a top-level drawer and closes it from the backdrop", async () => {
    const queryClient = new QueryClient({
      defaultOptions: {
        queries: { retry: false },
        mutations: { retry: false },
    ...
    expect(drawer).toHaveClass("z-[110]", "translate-x-0");
    expect(
      screen.getByLabelText("Kéo giãn chiều rộng khung chi tiết hợp đồng"),
    ).toBeInTheDocument();

    fireEvent.pointerDown(
      screen.getByTestId("service-contract-detail-drawer-backdrop"),
    );
    await waitFor(
      () =>
        expect(
          screen.queryByRole("dialog", { name: "Hợp đồng bảo trì" }),
  ...
  it("captures the legal number, standardized classification, value basis and original document during creation", async () => {
    ...
    vi.mocked(serviceContractsApi.upload).mockResolvedValue({
      id: "document-1",
    } as never);
    const queryClient = new QueryClient({
      defaultOptions: {
    ...
    const createForm = screen
      .getByRole("heading", { name: "Tạo hợp đồng dịch vụ" })
      .closest("form")!;
    fireEvent.change(
      within(createForm).getByLabelText(/^Số hợp đồng pháp lý/),
      { target: { value: "PL-2026-001" } },
    );
    fireEvent.change(within(createForm).getByLabelText(/^Tên hợp đồng/), {
      target: { value: "Hợp đồng bảo trì" },
    });
    fireEvent.change(within(createForm).getByLabelText(/^Tên đối tác/), {
      target: { value: "Đối tác" },
    });
    fireEvent.change(
      within(createForm).getByLabelText(/^Nhóm sản phẩm\/dịch vụ/),
      { target: { value: "MAINTENANCE" } },
    );
    await user.type(
      within(createForm).getByLabelText(/^Giá trị ban đầu/),
      "100000000",
    );
    await user.clear(within(createForm).getByLabelText(/^VAT/));
    await user.type(within(createForm).getByLabelText(/^VAT/), "20");
    expect(
      within(createForm).getByText("Giá trị hợp đồng này:", { exact: false }),
    ).toHaveTextContent("120.000.000");
    fireEvent.change(within(createForm).getByLabelText(/^Cơ sở giá trị/), {
      target: { value: "ANNUAL" },
    });
    fireEvent.change(within(createForm).getByLabelText(/^Ngày bắt đầu/), {
      target: { value: "2026-01-01" },
    });
    fireEvent.change(within(createForm).getByLabelText(/^Ngày kết thúc/), {
      target: { value: "2026-12-31" },
    });
    expect(createForm.querySelectorAll("[required]")).toHaveLength(9);
    expect(within(createForm).getAllByText("*", { exact: true })).toHaveLength(
      9,
    );
    within(createForm)
    ...
    const original = new File(["contract"], "hop-dong-goc.pdf", {
    ...
    await waitFor(() =>
      expect(serviceContractsApi.create).toHaveBeenCalledWith(
    ...
    await waitFor(() =>
      expect(serviceContractsApi.upload).toHaveBeenCalledWith(
        "contract-created",
        original,
        "CONTRACT",
      ),
    ...
    await waitFor(() =>
      expect(serviceContractsApi.alerts).toHaveBeenCalledTimes(2),
    );
  });
  ...
  it("hides edit and delete from a viewer the creator never shared with", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /deleteContract/ }),
    ).not.toBeInTheDocument();
  });
  ...
  it("offers delete inside the edit modal only at permission DELETE", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /deleteContract/ }),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByRole("tab", { name: /tabShare/ }));
    const editForm = screen
      .getByRole("heading", { name: "Chỉnh sửa hợp đồng" })
      .closest("form")!;
  ...
  it.skip("requires both dates and shows red required marks in the renew form", async () => {
    ...
    const renewForm = screen
      .getByRole("heading", { name: "Gia hạn hợp đồng" })
      .closest("form")!;
    expect(within(renewForm).getByLabelText(/^Ngày bắt đầu/)).toBeRequired();
    expect(within(renewForm).getByLabelText(/^Ngày bắt đầu/)).toHaveValue(
      "2026-12-31",
    );
    expect(
      within(renewForm).getByLabelText(/^Ngày kết thúc mới/),
    ).toBeRequired();
  ...
  it("does not offer manual expiring or expired transitions for an active contract", async () => {
    ...
    expect(
      await screen.findByText(
        /Sắp hết hạn và hết hạn được hệ thống tự động cập nhật/,
      ),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /Chuyển sang “Sắp hết hạn”/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /Chuyển sang “Hết hạn”/ }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: /Chuyển sang “Đã chấm dứt”/ }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Gia hạn" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("labels every recurring-payment field for the user", async () => {
    ...
    expect(screen.getByLabelText("Số tiền phải trả mỗi kỳ")).toBeDisabled();
    expect(screen.getByLabelText("Số tiền phải trả mỗi kỳ")).toHaveValue(
      "10.000.000",
    );
    expect(screen.getByLabelText("Ngày bắt đầu")).toHaveAttribute(
      "type",
      "date",
    );
    expect(screen.getByLabelText("Số kỳ")).toHaveDisplayValue("12");
    expect(screen.getByLabelText("Theo chu kỳ:")).toHaveTextContent(
      "Hàng tháng",
    );
    expect(screen.getByLabelText("Nhắc trước:")).toHaveDisplayValue("7");
    expect(screen.getByLabelText("Tên của mỗi kỳ")).toHaveValue(
      "Kỳ thanh toán",
    );
  });
  ...
  it("explains lifecycle choices and requires confirmation before changing status", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Chuyển sang “Đề xuất”/ }),
    );
    expect(serviceContractsApi.updateStatus).not.toHaveBeenCalled();
    fireEvent.click(
      screen.getByRole("button", { name: "Xác nhận chuyển trạng thái" }),
    );

    await waitFor(() =>
      expect(serviceContractsApi.updateStatus).toHaveBeenCalledWith(
        "contract-1",
        "PROPOSAL",
      ),
    );
  ...
  it("passes file metadata to the Service Contract open-or-download handler", async () => {
    ...
    await waitFor(() =>
      expect(openOrDownloadAuthenticatedDocument).toHaveBeenCalledWith(
...
describe("ServiceContractsPage — URL-driven contract selection", () => {
  ...
  it("opens the exact contract when the URL already carries ?id= on first load (fresh session / direct link / refresh)", async () => {
    vi.mocked(serviceContractsApi.detail).mockResolvedValue({
      ...
      shares: [],
      id: "svc-9",
      contractNumber: "PL-2026-009",
      title: "Hợp đồng vệ sinh",
      counterpartyName: "Đối tác",
      type: "CLEANING",
      status: "ACTIVE",
      totalValue: 5000000,
  ...
  it("fails gracefully for a nonexistent contract id instead of breaking the page", async () => {
    vi.mocked(serviceContractsApi.detail).mockRejectedValue(
      new Error("Not Found"),
    );
    ...
    expect(
      await screen.findByText("Không tìm thấy hợp đồng"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Hợp đồng dịch vụ này không tồn tại hoặc đã bị xóa."),
    ).toBeInTheDocument();
  });
  ...
  it("closing the detail removes only ?id, preserving every other query param", async () => {
    ...
    await screen.findByText(
      (_, node) => {
        ...
        return (
          text.includes("/service-contracts?") &&
          !text.includes("id=svc-9") &&
