...
| 33 | Lock unit khi booking ACTIVE | Unit.status → BOOKING khi activate | 🟡 | Verify service transition có chuyển UnitStatus | Meeting: *"status đang thương thảo là lock sảnh k cho book"* |
| 34 | 1 booking gồm nhiều dịch vụ (bỏ/thêm khi trình) — cho short-term | `SlotBooking` = 1 slot / booking, chưa có line items | ❌ | Model `BookingLine[]` (slot + service + qty + price) để 1 booking gộp nhiều slot/service | Meeting: *"1 HĐ book nhiều dịch vụ -> có thể bỏ or thêm dịch vụ khi trình ms. oanh"* |
| 35 | Cross-department handoff (a.Biên báo a.Sơn để book) | `BookingActivity` có 7 activity type, chưa có handoff | ❌ | Thêm `BookingActivityType.INTER_DEPT_REQUEST` + assign notification | Meeting: *"a Biên phải liên hệ a Sơn để booking chứ k tự ý book"* |
...
| 38 | Booking hết hạn tự động release | `holdDays`, `expiresAt` | 🟡 | Verify có cron scan expire → release | Suy luận (best practice) |

### 2.2 Tab ĐỀ XUẤT (`/proposals`) — Tờ Trình

| # | Yêu cầu | Hiện tại | Trạng thái | Đề xuất | Nguồn |
|---|---|---|---|---|---|
| 39 | Thư đề nghị cho thuê (OTL — Offer To Lease) tách biệt | Chỉ có `Proposal` chung, không phân biệt OTL / TIF | ❌ | Thêm `Proposal.documentType`: `OTL` \| `TIF` \| `TO_TRINH_XAC_NHAN` \| `TO_TRINH_DIEU_CHINH` | Excel KT (cột "OTL" tách với "LA") + Meeting: *"Thương thảo thư đề nghị (ndung)"* |
| 40 | Tờ Trình xác nhận thuê — 21 mục theo template | `Proposal` field có: `businessModel`, `serviceFeeSqm`, `businessSupportFeeSqm`, `rentCurrency`, `fitoutDays`, `handoverDate`, `openingDate`, `specialConditions`, `editorContent` | 🟡 | Verify đủ 21 mục: (1) Pháp nhân, (2) Thương hiệu, (3) Ngành hàng, (4) Mô hình KD, (5) Vị trí, (6) Diện tích, (7) Thời hạn thuê, (8) Ngày tính tiền thuê, (9) Giá thuê, (10) Phí DV, (11) Phí hỗ trợ KD, (12) Phí tiện ích, (13) Giờ hoạt động, (14) Phí ngoài giờ, (15) Tỷ giá, (16) Thanh toán, (17) Đặt cọc, (18) Thời hạn hoàn thiện, (19) Phí thi công, (20) Ngày bàn giao MB, (21) ĐK đặc biệt | Tờ Trình XN (21 mục nguyên văn) |
| 41 | Cọc thuê + Cọc thi công + Phí thi công (3 khoản riêng) | `Contract.deposit` (1 field); `Proposal.deposit` (số tháng) | ❌ | Tách: `depositLease` (tiền cọc thuê = 3 tháng), `depositFitout` (cọc thi công), `fitoutFee` (phí thi công) trên cả Proposal & Contract | Excel KT (3 cột: "CỌC ĐẢM BẢO", "CỌC THI CÔNG", "PHÍ THI CÔNG") + Tờ Trình XN mục 17 & 19 |
| 42 | Tỷ giá VND/USD ghi vào Tờ Trình | `Proposal.rentCurrency` | 🟡 | Thêm `Proposal.exchangeRate Float?` + `exchangeRateSource` (VCB, ...) | Tờ Trình XN mục 15: *"tỷ giá bán ra của Ngân hàng TMCP Ngoại Thương VN... 26.340 VND/USD"* |
...
| 44 | Xuất PDF Proposal / Tờ Trình / TIF | **Chưa có** (IMPLEMENTATION.md § Known Limitations) | ❌ | Dùng `pdfmake` hoặc `puppeteer`; template render đúng layout Tờ Trình + TIF | Tờ Trình XN + TIF (đều là văn bản chính thức) |
...
| 46 | Giá lệch quy định → trigger duyệt Ms.Oanh | Có `discount` field trigger workflow, chưa có "lệch range policy" | 🟡 | Thêm `ApprovalPolicyRule.conditionType = 'PRICE_OUT_OF_RANGE'` so với `CategoryMallPricing.min/max` | Meeting: *"giá thuê trong mức quy định thì k cần ms.Oanh duyệt, giá ngoài quy định cần ms.Oanh duyệt"* |
...
| 51 | Tờ Trình điều chỉnh chốt thuê (pre-signing) | `ContractAmendment` có nhưng dành cho post-signing | ❌ | Thêm `ProposalAdjustment` hoặc extend `ContractAmendment.stage = PRE_SIGN\|POST_SIGN` | Tờ Trình ĐC (file `20260525_TTr Dieu chinh thong tin chot thue.docx` — điều chỉnh Ngày Bàn giao, Ngày Khai trương trước ký HĐ) |
...
| Priority | Nhóm | Items |
|---|---|---|
| **P0** — Chặn nghiệp vụ | Approval flow, RBAC, cấu trúc tài chính | #41 (tách 3 khoản deposit), #52-53 (2 workflow dài/ngắn), #55 (skip CEO khi trong range), #79-82 (RBAC scope) |
| **P1** — Output nghiệp vụ | PDF, Excel export, template | #40 (verify 21 mục Tờ Trình), #43 (layout attach), #44 (PDF export), #51 (Tờ Trình điều chỉnh pre-sign), #76 (Excel báo cáo đánh giá) |
...
| **P3** — Booking ngắn hạn & cảnh báo | Line items, cron alerts | #14 (existing tenant discount), #34 (BookingLine[]), #35 (cross-dept handoff), #24 (cron HĐ hết hạn), #25 (cron thi công trễ) |
| **P4** — Nâng cấp | Tenant scoring, eSign, blacklist | #63 (eSign), #66 (auto penalty policy), #77 (TenantScore), #78 (blacklist) |
...
- #33 `bookings.service.ts` transition unit status → BOOKING
- #38 Cron scan booking expired
- #40 `Proposal.editorContent` có đủ 21 field
